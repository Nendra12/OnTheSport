        ...
        <div class="flex justify-center border-b-1 border-[#d6dbdf] p-4">
            <div class="flex gap-4 text-gray-700">
                ...
                @foreach ($lists as $list)
                    <a href="#" class="pr-4 border-r border-gray-300 hover:text-black">{{ $list->nama }}</a>
                @endforeach
            </div>
        </div>
    </nav>
    <article class="flex mx-4 flex-col font-sans">
        @php $groupedPosts = $posts->chunk(3); @endphp
        
        @foreach ($groupedPosts as $group)
            {{-- Normal Posts (First 2 in group) --}}
            @if ($group->count() >= 2)
            ...
            {{-- Featured Post (3rd in group) --}}
            @if ($group->count() == 3)
                @php $featuredPost = $group->last(); @endphp
                <div class="bg-[#ece8e8] p-5">
                    <a href="#" class="flex justify-between gap-5 items-center group">
                        <img src="{{ asset('storage/' . $featuredPost->gambar) }}" alt="" width="55%" class="group-hover:opacity-85">
                        <div class="flex flex-col gap-5">
                            <h3 class="text-2xl group-hover:underline">{{ $featuredPost->judul }}</h3>
                            <p class="font-normal">{!! $featuredPost->isi !!}</p>
                            <div class="font-normal flex gap-4 mt-2 text-xs font-[Verdana]">
                                <p class="pr-4">{{ $featuredPost->created_at }}</p>
                            </div>
                        </div>
                    </a>
                </div>
